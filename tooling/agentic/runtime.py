...
class JarvisAgenticRuntime:
    ...
    def execute_goal(
        ...
        for wave in waves:
            ...
            for task in wave.tasks:
                ...
                action = "command" if cmd_to_run else "execute_task"
                ...
                # Capture artifacts from write scopes
                for wscope in task.write_scopes:
                    wp = Path(wscope)
    ...
    def resume_mission(self, mission_id: str, operator_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Re-schedule remaining unfinished tasks
        waves = self.scheduler.schedule(mission.dag)
        mission.status = MissionStatus.RUNNING
        self.state_store.save_mission(mission)
