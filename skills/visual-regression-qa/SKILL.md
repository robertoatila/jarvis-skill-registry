...
description: Automates visual regression testing across browsers, responsive viewports, and themes.
---
...
node script/qa/web-terminal-visual-qa.mjs --title "TUI Visual QA" \
  ...
  --evidence-dir ${EVIDENCE_DIR:-.evidence}/<slug>/tui-web-terminal
```
...
```
node "$SKILL_DIR/scripts/visual-qa.mjs" tui-check ${EVIDENCE_DIR:-.evidence}/<slug>/tui-web-terminal/terminal.txt --cols <N>
```
...
1. Inspect project scope and load relevant configuration.
2. Execute structured analysis according to rules.
3. Produce verifiable remediation evidence.
